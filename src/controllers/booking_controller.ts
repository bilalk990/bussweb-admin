...
export const bookingController = {
    ...
    createBooking: async (req: Request, res: Response) => {
        try {
            ...
            const totalPrice = seats.length * (route.adultPrice || 0);
    ...
    cancelBooking: async (req: Request, res: Response) => {
        try {
            ...
            const hoursUntilDeparture = (departureDateTime.getTime() - new Date().getTime()) / (1000 * 60 * 60);
            let refundAmount = 0;

            if (hoursUntilDeparture > 24) {
                refundAmount = (booking.totalPrice || 0) * 0.8; // 80% refund
            } else if (hoursUntilDeparture > 12) {
                refundAmount = (booking.totalPrice || 0) * 0.5; // 50% refund
            }
