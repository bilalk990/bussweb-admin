...
type OperationChanges = { field: string; oldValue?: any; newValue?: any }[];
...
export const staffOperationMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const originalUserInstance = await User.findByPk(targetUserId);
    const originalUser = originalUserInstance?.get({ plain: true }) as any;
    ...
    res.json = function (body) {
        ...
        (async () => {
            try {
                ...
                const updatedUser = updatedUserInstance?.get({ plain: true }) as any;
